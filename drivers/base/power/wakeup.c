...
static bool enable_qpnp_fg_sanity_check_ws = true;
module_param(enable_qpnp_fg_sanity_check_ws, bool, 0644);
        ...
        if (ws) {
			...
			(!enable_wlan_ipa_ws &&
				!strncmp(ws->name, "wlan_ipa", wslen)) ||
                        (!enable_qpnp_fg_sanity_check_ws &&
				!strncmp(ws->name, "qpnp_fg_sanity_check", wslen)) ||
